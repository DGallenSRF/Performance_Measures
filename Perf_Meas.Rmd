...
plot_TTI <- function(data, offset, location){
  ...
  max_TTI <- round(max(dat$TTI),2)
  min_TTI <- round(min(dat$TTI),2)
  ...
  ggplot(dat, aes(x = DateTime , y = TTI)) + 
    ...
    annotate('text',x = min(dat$DateTime), y = Quant + offset, family = 'serif',
             label = paste('90th PCTL:', Quant), hjust = 0) +
    scale_y_continuous(breaks = y_breaks) +
    scale_x_datetime(breaks = x_breaks,
                     labels = scales::date_format('%H:%M', tz = 'US/Central')) +
    labs(title = paste('TTI vs Time on',
                  paste(weekdays(as.POSIXct(select_Day, format=('%m/%d/%Y'))),
                        ',',sep=''),
                  select_Day, location),
...
plot_TTI(dat_tmc,0.04, location = '\nUS-212, Chaska')
```

```{r,fig.height = 4, fig.width = 10}
select_Day <- '03/30/2017'
dat_tmc <- dat_494 %>%
...
dat_cal_494_125 <- dat_cal_494(1.25)
...
dat_cal_169_125 <- dat_cal_169(1.25)
...
ggplot(dat_range, aes(Speed,Hour, group = Hour, fill = ..x..)) +
  ...
  viridis::scale_fill_viridis(name = "TT", option = 'C') +
